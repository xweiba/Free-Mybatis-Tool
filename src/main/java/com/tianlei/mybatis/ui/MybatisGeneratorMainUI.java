...
public class MybatisGeneratorMainUI extends JFrame {
    ...
    private final JTextField modelPostfixField = new JTextField(10);

    private final JTextField daoPrefixField = new JTextField(10);
    private final JTextField modelPrefixField = new JTextField(10);
    ...
    private final JCheckBox jpaAnnotationBox = new JCheckBox("JPA-Annotation(JPA注解)");
    ...
    private boolean isAutoGeneratedPrimaryKey = false;
    ...
    public MybatisGeneratorMainUI(AnActionEvent anActionEvent) throws HeadlessException {
        ...
        if (psiElements.length > 1) {
            setPreferredSize(new Dimension(1300, 550));//设置大小
        } else {
            setPreferredSize(new Dimension(1300, 650));//设置大小
        }
        setLocation(300, 300);
        pack();
        setVisible(true);
        ...
        if (tableInfo.getPrimaryKeys().size() > 0) {
            ...
            for (DasColumn column : tableInfo.getColumns()) {
                if (column.getName().equals(primaryKey)) {
                    isAutoGeneratedPrimaryKey = DasUtil.isAuto(column);
                    break;
                }
            }
        }
    ...
    private void onOK() {
        try {
            ...
            if (psiElements.length == 1) {
                ...
                generator_config.setDaoName(daoPrefixField.getText() + daoNameField.getText() + daoPostfixField.getText());
                generator_config.setModelName(modelPrefixField.getText() + modelNameField.getText() + modelPostfixField.getText());
                generator_config.setDaoPrefix(daoPrefixField.getText());
                generator_config.setDaoPostfix(daoPostfixField.getText());
                generator_config.setModelPrefix(modelPrefixField.getText());
                generator_config.setModelPostfix(modelPostfixField.getText());

                generator_config.setPrimaryKey(keyField.getText());
                generator_config.setAutoGeneratedPrimaryKey(isAutoGeneratedPrimaryKey);
                ...
                for (PsiElement psiElement : psiElements) {
                    ...
                    String modelName = StringUtils.dbStringToCamelStyle(tableName);
                    String primaryKey = "";
                    boolean isAutoGeneratedPrimaryKeyTemp = false;
                    if (tableInfo.getPrimaryKeys() != null && tableInfo.getPrimaryKeys().size() != 0) {
                        primaryKey = tableInfo.getPrimaryKeys().get(0);

                        for (DasColumn column : tableInfo.getColumns()) {
                            if (column.getName().equals(primaryKey)) {
                                isAutoGeneratedPrimaryKeyTemp = DasUtil.isAuto(column);
                                break;
                            }
                        }
                    }
                    ...
                    generator_config.setDaoName(daoPrefixField.getText() + modelName + daoPostfixField.getText());
                    generator_config.setModelName(modelPrefixField.getText() + modelName + modelPostfixField.getText());
                    ...
                    generator_config.setAutoGeneratedPrimaryKey(isAutoGeneratedPrimaryKeyTemp);
    ...
    private JPanel initModelSetting(boolean multiTable, String modelName) {
        ...
        if (config != null && !StringUtils.isEmpty(config.getModelPrefix())) {
            modelPrefixField.setText(config.getModelPrefix());
        } else {
            modelPrefixField.setText("");
        }
        modelNameFieldPanel.add(new JLabel("prefix:"));
        modelNameFieldPanel.add(modelPrefixField);
        ...
        modelNameFieldPanel.add(new JLabel("postfix:"));
        modelNameFieldPanel.add(modelPostfixField);
    ...
    private JPanel initDaoSetting(boolean multiTable, String modelName) {
        ...
        if (config != null && !StringUtils.isEmpty(config.getDaoPrefix())) {
            daoPrefixField.setText(config.getDaoPrefix());
        } else {
            daoPrefixField.setText("");
        }
        ...
        daoNameFieldPanel.add(new JLabel("prefix:"));
        daoNameFieldPanel.add(daoPrefixField);

        daoNameFieldPanel.add(new JLabel("postfix:"));
        daoNameFieldPanel.add(daoPostfixField);

        if (!multiTable) { //单表
            daoNameField.setText(modelName);
