...
public class MybatisGenerator {
    ...
    private TableConfiguration buildTableConfig(Context context) {
        ...
        if (!StringUtils.isEmpty(config.getPrimaryKey())) {
            if ((DbType.MySQL.equals(dbType) || DbType.MariaDB.equals(dbType)) && config.isAutoGeneratedPrimaryKey()) {
...
//        }
    ...
    private void addPluginConfiguration(Context context) {
        ...
        if (config.isUseJpaAnnotation()) {
            // 添加 @Column @Table注解
            PluginConfiguration pluginConfiguration = new PluginConfiguration();
            pluginConfiguration.addProperty("type", "com.tianlei.mybatis.generate.plugin.JpaAnnotationPlugin");
